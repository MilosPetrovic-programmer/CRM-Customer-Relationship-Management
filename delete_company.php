...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    $stmt = $con->prepare("DELETE FROM kompanije WHERE id = ?");
    if ($stmt) {
        ...
        if ($stmt->affected_rows > 0) {
                redirect("index.php");
                exit();
            //echo "Kompanija uspešno obrisan.";
            $response['status'] = 'success';
            $response['message'] = 'Kompanija je uspešno obrisana.';
        } else {
            //echo "Greška prilikom brisanja kompanije ili kompanija ne postoji.";
            $response['status'] = 'error';
            $response['message'] = 'Greška prilikom brisanja kompanije ili kompanija ne postoji.';
        }      
    } else {
